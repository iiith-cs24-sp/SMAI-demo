...
def detect_corners(image_path):
    print(f"Loading corner detection model...")
...
def chess_pieces_detector(image):
    ...
    print(f"Loading chess piece detection model...")
    # Use absolute paths to ensure model loading works correctly
    model_path = os.path.abspath(os.path.join("models", "piece-recog-shiram-chessrender360.pt"))
    if not os.path.exists(model_path):
    ...
    with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp:
        ...
        image.save(tmp.name)
        print(f"Running piece detection with confidence threshold 0.25...")
    ...
    for i, box in enumerate(detections):
        ...
        if class_names and boxes.cls is not None:
            ...
            plt.text(x1, y1-5, label, fontsize=8, 
...
def main():
    ...
    for line in FEN_annotation:
        ...
        corrected_FEN = line_to_FEN  # No need to replace 'empty' as we're using '1' directly
        print(corrected_FEN)
